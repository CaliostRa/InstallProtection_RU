...
class SourceLocalizationTest(unittest.TestCase):
    def test_package_and_xposed_entry_points_are_unchanged(self):
        ...
        fork_version_code = int(version_code.group(1))
        self.assertGreater(fork_version_code, 241)
        self.assertIsNotNone(version_name)
        fork_version_name = version_name.group(1)
        revision = re.fullmatch(r"2\.1\.68-ru(\d+)", fork_version_name)
        self.assertIsNotNone(revision)
        revision_number = int(revision.group(1))
        self.assertGreaterEqual(revision_number, 1)
        self.assertLessEqual(revision_number, 99)
        self.assertEqual(24100 + revision_number, fork_version_code)
